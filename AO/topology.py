...
class Convolutional:
	...
	def build_layer(self, model):
		model.add(keras.layers.Conv1D(filters=self.filters,
...
class Pooling:
	...
	def build_layer(self, model):
		if self.name == 'MaxPooling1D':
			model.add(keras.layers.MaxPooling1D(self.pool_size, self.stride_size, self.padding))
		elif self.name == 'AveragePooling1D':
			model.add(keras.layers.AveragePooling1D(self.pool_size, self.stride_size, self.padding))
