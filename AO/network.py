...
class Network:
        ...
        for block in self.block_list:
            for layer in block.get_layers():                # build model
                try:
                    ...
                    print('Individual abborted due to Exception: ', exp)
    ...
    def train_and_evaluate(self, model, dataset):
        ...
        history = model.fit(dataset['x_train'],dataset['y_train'],batch_size=dataset['batch_size'],epochs=dataset['epochs'],validation_data=(dataset['x_valid'], dataset['y_valid']),shuffle=True)
        ...
        model.save('all_models/'+self.name + '.h5')                       # save model
        save_network(self)                                  # save topology, model and fitness
        
    def asexual_reproduction(self, it, dataset, recreate_models):

        # if the individual already exists, just load it
        if os.path.isfile('all_models/'+'net_' + str(it) + '.h5') and not(recreate_models):
            print("\n-------------------------------------")
        ...
        if model == -1:
            return self.asexual_reproduction(it, dataset, recreate_models)
    ...
    def block_mutation(self, dataset):
        ...
        if len(bl) == 0:
            ...
            layerList1 = [
                Convolutional(filters=pow(2, randint(1,5)),
                              filter_size=(3),
                              stride_size=(1),
                              padding='same',
        ...
        if mutation_type:                                       # remove
            ...
            if block_type_idx:
                ...
                layer = Convolutional(filters=pow(2, randint(1,5)),
                                      filter_size=(3),
                                      stride_size=(1),
                                      padding='same',
                                      input_shape=dataset['x_train'].shape[1:])
    ...
    def layer_mutation(self, dataset):
        ...
        if layer.name == 'Conv1D':
            ...
            del layerList[idx]
            conv_layer = Convolutional(filters=pow(2, randint(1,5)),
                                       filter_size=(3),
                                       stride_size=(2),
